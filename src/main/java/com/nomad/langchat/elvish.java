...
public class elvish implements CommandExecutor {

    private final Main main;

    public elvish(Main main){
        this.main = main;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if(main.getConfig().getBoolean("Settings.Elvish.Enabled")) {
                ...
                if (args.length >= 1) {
                    ...
                    for (Entity entity : player.getNearbyEntities(main.getConfig().getInt("Settings.Elvish.Distance"), main.getConfig().getInt("Settings.Elvish.Distance"),main.getConfig().getInt("Settings.Elvish.Distance")) ){
                        if (entity instanceof Player) {
                            Player otherplayer = (Player) entity;
                            //check if players have the right permission
                            if(otherplayer.hasPermission(main.getConfig().getString("Settings.Elvish.Permission"))){
                                //if so send this message
                                otherplayer.sendMessage(player.getName() + ChatColor.GREEN + " [Elvish] " + ChatColor.WHITE + builder);
                        ...
                        player.sendMessage(player.getName() + ChatColor.GREEN + " [Elvish] " + ChatColor.WHITE + builder);
                    }
                } else {
                    //error message for wrong santex
                    sender.sendMessage(ChatColor.RED + "Invalid usage! Use /Elvish <message> OR /el <message>");
                }
            } else {
                player.sendMessage("Elvish is not enabled in the config");
            }
            return false;
        }
        return false;
